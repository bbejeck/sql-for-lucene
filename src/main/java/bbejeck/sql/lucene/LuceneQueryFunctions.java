...
import bbejeck.sql.util.Collectors;
import bbejeck.sql.util.ThrowingBiFunction;
...
public interface LuceneQueryFunctions {
    ...
    Function<BooleanQuery,BooleanClause> extractBooleanClause = bq -> bq.clauses().iterator().next();
    ...
    BiFunction<String, String, PhraseQuery> toPhraseQuery = (field, text) -> {
        ...
        toWordStream.apply(text).map(termFunction.apply(field)).forEach(builder::add);
        return builder.build();
    };

    Function<List<BooleanClause>, BooleanQuery> toBooleanQuery = l -> {
